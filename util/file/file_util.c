...
FILE *file_fn(const char fn[], const char op_type[], int line_num,
  ...
  if (NULL == fn) {
    __errorf(line_num, func_name, file_name, "Unable to read file.");
  }
  if (NULL == op_type) {
    __errorf(line_num, func_name, file_name, "Unable to read file '%s'.", fn);
  }
  FILE *file = fopen(fn, op_type);
  if (NULL == file) {
    __errorf(line_num, func_name, file_name,
